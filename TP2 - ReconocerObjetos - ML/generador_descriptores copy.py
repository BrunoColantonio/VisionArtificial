...
binary_square = cv2.threshold(gray_square,50,255,cv2.THRESH_BINARY)[1]
sq_contours, sq_hierachy = cv2.findContours(binary_square, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
references["square"] = sq_contours[0]

triangle = cv2.imread("./shapes/triangulo.png")
gray_triangle = cv2.cvtColor(triangle, cv2.COLOR_BGR2GRAY)
binary_triangle = cv2.threshold(gray_triangle,50,255,cv2.THRESH_BINARY)[1]
tr_contours, tr_hierachy = cv2.findContours(binary_triangle, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
references["triangle"] = tr_contours[0]

circle = cv2.imread("./shapes/circulo.jpg")
gray_circle = cv2.cvtColor(circle, cv2.COLOR_BGR2GRAY)
binary_circle = cv2.threshold(gray_circle,50,255,cv2.THRESH_BINARY)[1]
cir_contours, cir_hierachy = cv2.findContours(binary_circle, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
